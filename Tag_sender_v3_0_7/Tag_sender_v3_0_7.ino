/*
 * =====================================================
 * TAG SENDER v3.0.7 - GPS Trilateration + Bidirectional
 * =====================================================
 * BASE: v3.0.6 (Trilateration Added)
 * NEW FEATURES:
...
static const char* FW_VERSION = "v3.0.7-BiDir";
...
// ⭐ โครงสร้างรับคำสั่งจาก Node (Downlink) - เพิ่มใหม่
typedef struct __attribute__((packed)) {
...
void drawScreen(float rawA1, float rawA2, float rawA3,
  ...
  display.printf("TAG-%d %s", TAG_ID, getRangeLabel(getDistanceRange(rawA1)));
...
void setup() {
  ...
  Serial.println("\n=== TAG SENDER v3.0.7 (BiDir + GPS) ===");

  // 1. ⚠️ แก้ปัญหา WiFi Connecting Error (จากรูปที่คุณส่งมา)
